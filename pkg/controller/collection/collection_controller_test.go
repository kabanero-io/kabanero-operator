...
var cImages []kabanerov1alpha1.Image = []kabanerov1alpha1.Image{
	{Id: "a1b22b1a", Image: "docker.io/kabanero/java-microprofile:0.2"}}
		...
		Versions: []kabanerov1alpha2.StackVersion{
			{
				...
				Images:               []kabanerov1alpha2.Image{{Id: "a1b22b1a", Image: "docker.io/kabanero/java-microprofile"}},
			},
			{
				Version:              "4.5.6",
				SkipCertVerification: true,
...
func TestConvertToStackUsingCollectionWithOneVersionHasEmptyPipelineAndImage(t *testing.T) {
	...
	if stack.Spec.Versions[1].Images[0].Image != "docker.io/kabanero/java-microprofile" {
		t.Fatal(fmt.Sprintf("The resulting stack.Spec.Versions[1].Images[0].Id is not docker.io/kabanero/java-microprofile as expected. It instead shows: " + stack.Spec.Versions[0].Images[0].Image))
	}

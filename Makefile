...
build-image: generate
  ...
  # static.
	...
	cp deploy/crds/kabanero_kabanero_crd.yaml deploy/crds/kabanero_collection_crd.yaml build/registry/manifests/kabanero-operator/0.3.2/

ifdef INTERNAL_IMAGE
  # Deployment uses internal registry service address
	sed -e "s!kabanero/kabanero-operator:latest!${INTERNAL_IMAGE}!" registry/manifests/kabanero-operator/0.3.2/kabanero-operator.v0.3.2.clusterserviceversion.yaml > build/registry/manifests/kabanero-operator/0.3.2/kabanero-operator.v0.3.2.clusterserviceversion.yaml
else
	sed -e "s!kabanero/kabanero-operator:latest!${IMAGE}!" registry/manifests/kabanero-operator/0.3.2/kabanero-operator.v0.3.2.clusterserviceversion.yaml > build/registry/manifests/kabanero-operator/0.3.2/kabanero-operator.v0.3.2.clusterserviceversion.yaml
endif
...
ifdef TRAVIS_TAG
	sed -e "s!kabanero/kabanero-operator:latest!${REPOSITORY}:${TRAVIS_TAG}!" registry/manifests/kabanero-operator/0.3.2/kabanero-operator.v0.3.2.clusterserviceversion.yaml > build/registry/manifests/kabanero-operator/0.3.2/kabanero-operator.v0.3.2.clusterserviceversion.yaml
	docker build -t ${REGISTRY_REPOSITORY}:${TRAVIS_TAG} -f build/registry/Dockerfile build/registry/
endif

ifdef TRAVIS_BRANCH
	sed -e "s!kabanero/kabanero-operator:latest!${REPOSITORY}:${TRAVIS_BRANCH}!" registry/manifests/kabanero-operator/0.3.2/kabanero-operator.v0.3.2.clusterserviceversion.yaml > build/registry/manifests/kabanero-operator/0.3.2/kabanero-operator.v0.3.2.clusterserviceversion.yaml

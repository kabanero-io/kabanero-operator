...
if [ `oc get deploymentConfig -n kabanero --output=jsonpath='{range .items[0]}{"\n"}{@.metadata.name}{end}' --ignore-not-found | wc -l` -gt 0 ] ; then
   #official final URL; haven't been tested yet
   # bash |  curl  https://raw.githubusercontent.com/kabanero-io/kabanero-operator/${RELEASE}/deploy/keycloak-uninstall.sh  
    #for testing with private branch before using official URL
    bash |  curl https://github.com/meiaus/kabanero-operator/tree/master/deploy/keycloak-uninstall.sh
